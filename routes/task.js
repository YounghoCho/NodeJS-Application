                ...
                if (error) {
                    ...
                    if (rows == "D") {
                        ...
                        if (!(home_lat && home_long && workplace_lat && workplace_long)) {
                            ...
                            let value = ["0000-00-00 00:00:00", lat, long, lat, r];
...
router.post('/', function(req, res) {
    pool.getConnection(function(error, connection) {
        if (error) {
            ...
            connection.query(selectQuery, function(error, rows) {
                if(error) {
                    console.log("Connection Error0 : " + error);
                    res.sendStatus(500).send({ message: "Connection Error0 : " + error });
                    connection.release();
                } else {
                    if (rows == "A"){
                        ...
                        if (!(task_type && cost && details && deadline && workplace_lat && workplace_long
                            ...
                            connection.query(insertQuery, value, function(err, rows) {
                                if(err) {
                                    ...
                                    res.sendStatus(500).send("Connection Error1 : " + err);
                                    connection.release();
                                } else {
                            ...
                            connection.query(updateQuery, value, function(error, rows) {
                                if(error) {
                                    ...
                                    res.sendStatus(500).send("Connection Error2 : " + err);
                                    connection.release();
                                }
                                else {
                                    res.status(200).json({ message : "Success in getting user information" });
